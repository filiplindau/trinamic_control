...
#		print 'Status',data.status
	...
	def setMaxCurrent(self, current, motor=0):
		...
		value = int(current * 1000.0)		 # Value in mA (wrong in manual) 
		self.sendCommand(cmd, type, motor, value)
		data = self.receiveData()
		cmd = 'SAP'	   # Get axis parameter
		type = 7		  # Standby current 
		value = int(current * 1000.0 * 0.1)		 # Value in mA (wrong in manual) 
...
#		print 'Status',data.status
		if data.status != 100:
			if self.errorDict.has_key(data.status):
				...
				raise MotorError(''.join(('Unknown error, ', str(data.status))))
		current = data.value / 1000.0 # Value in mA (wrong in manual)
		return current		
	
	def setIdleCurrent(self, idleCurrent, motor=0):
		cmd = 'SAP'	   # Get axis parameter
		type = 7		  # Standby current 
		value = int(idleCurrent * 1000.0)		 # Value in mA (wrong in manual) 
	...
	def startReferenceSearch(self, motor=0):
		...
		if data.status != 100:
			if self.errorDict.has_key(data.status):
				...
				raise MotorError(''.join(('Unknown error, ', str(data.status))))
		speed = self.getTargetSpeed(motor)
		cmd = 'SAP'	   # Set axis parameter
		type = 194		# Ref search speed
		value = 1	 # Use half speed as normal running
		self.sendCommand(cmd, type, motor, value)
		data = self.receiveData()
		if data.status != 100:
			if self.errorDict.has_key(data.status):
		...
		value = 2	 # Use quarter speed as normal running
		self.sendCommand(cmd, type, motor, value)
		data = self.receiveData()
		if data.status != 100:
			if self.errorDict.has_key(data.status):
